            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h4 class="card-title mb-0"> Edit & Update Navigation portfolio</h4>
                        <a href="{{ route('admin.portfolio.index')}}" class="btn btn-primary"> portfolio List</a>
                    </div>
                    <!-- end card header -->
                                ...
                                <form method="POST"  action="{{ route('admin.portfolio.update', $portfolio->id) }}" enctype="multipart/form-data">
                                    ...
                                    @method('PUT')
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control" placeholder="Title" value="{{ $portfolio->title }}" name="title" id="title" required>
                                        </div>
                                        <div class="mb-3">
                                            <label for="category-dropdown" class="form-label">Portfolio Category</label>
                                            <select class="form-select" id="category-dropdown" name="category_id">
                                                <option value="" disabled selected>Select category</option>
                                                @foreach ($portfolioCategory as $item)
                                                    <option value="{{ $item->id }}" {{ $portfolio->category_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="mb-3">
                                            <label for="labelInput" class="form-label">Content</label>
                                            <textarea type="text" class="ckeditor-classic" placeholder="Answer" name="content" id="content" required>{{ $portfolio->content }}</textarea>
                                        </div>
                                        <div class="mb-3 row ">
                                            ...
                                            <div class="col-sm-9">
                                                ...
                                                @enderror
                                                <img src="{{ asset($portfolio->image) }}" alt="{{ $portfolio->title }}" class="img-thumbnail mt-2" width="200">
