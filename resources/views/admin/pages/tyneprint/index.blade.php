                            ...
                            <div class="table-responsive table-card mt-3 mb-1">
                                ...
                                <table class="table align-middle table-nowrap" id="menuTable">
                                    ...
                                    <tbody class="list form-check-all">
                                        @forelse ($tyneprint as $index => $tyneprint)
                                        <tr>
                                            <td><strong>{{  $index + 1 }}</strong></td>
                                            <td class="menu_name">{{ $tyneprint->title }}</td>
                                            <td class="menu_name">{!! Str::limit($tyneprint->content, 30) !!}</td> 
                                            <td>
                                                <img src="{{ asset($tyneprint->image) }}" class="img-thumbnail" height="30" alt="{{ $tyneprint->title }}"  style="max-width: 100px;"/>
                                            </td>
                                            <td class="menu_name">{{ \Carbon\Carbon::parse($tyneprint->created_at)->format('d M Y') }}</td> 

                                            <td>
                                                <div class="d-flex gap-2">
                                                    <div class="edit">
                                                        <a href="{{ route('admin.tyneprint.edit',  encrypt($tyneprint->id) ) }}" class="btn btn-sm btn-success edit-item-btn" >Edit</a>
                                                    </div>
                                                    <div class="remove">
                                                        <button class="btn btn-sm btn-danger remove-item-btn" data-bs-toggle="modal" data-bs-target="#deleteRecordModal{{ $tyneprint->id }}" data-id="{{ $tyneprint->id }}">Remove</button>
                                        ...
                                        {{-- Delete --}}
                                        <div class="modal fade zoomIn" id="deleteRecordModal{{ $tyneprint->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="mt-2 text-center">
                                                            ...
                                                            <div class="mt-4 pt-2 fs-base mx-4 mx-sm-5">
                                                                <h4>Are you Sure ?</h4>
                                                                <p class="text-muted mx-4 mb-0">Are you sure you want to remove this tyneprint?</p>
                                                            </div>
                                                        </div>
                                                        <div class="d-flex gap-2 justify-content-center mt-4 mb-2">
                                                            <button type="button" class="btn w-sm btn-light" data-bs-dismiss="modal">Close</button>
                                                            <form method="POST" action="{{ route('admin.tyneprint.destroy', encrypt($tyneprint->id) ) }}">
                                                                @csrf
                                                                @method('DELETE')
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header bg-light p-3">
                        <h5 class="modal-title" id="exampleModalLabel">Add TynePrint</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" id="close-modal"></button>
                    </div>
                    <form class="tablelist-form"  method="POST" action="{{ route('admin.tyneprint.store') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="modal-body">
                            <div class="mb-3">
